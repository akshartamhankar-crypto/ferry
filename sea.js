(function () {
  ...
  function initRain() { drops.length = 0; var n = reduce ? 0 : Math.min(mobile ? 110 : 480, Math.floor(W / (mobile ? 7 : 2.5))); for (var i = 0; i < n; i++) drops.push(mkDrop(true)); }
  ...
  function drawClouds(off) {
    for (var i = 0; i < clouds.length; i++) {
      var c = clouds[i]; c.x += c.sp * 0.016; if (c.x > W + 180) c.x = -180; var y = c.y - off;
      var fg = c.layer === 1, lit = flash > 0.3;
      ctx.save(); ctx.globalAlpha = fg ? 0.94 : 0.6;
      for (var b = 0; b < 6; b++) {
        var bx = c.x + (b - 2.5) * 38 * c.s, by = y + Math.sin(b * 1.3 + c.x * 0.01) * 9, br = (32 + (b % 3) * 15) * c.s;
        var top = lit ? (fg ? '#6f828a' : '#7e9199') : (fg ? '#3c4f56' : '#52656c');
        var bot = lit ? (fg ? '#4a5d64' : '#56686f') : (fg ? '#283840' : '#3a4c53');
  ...
  function drawFog(off) {
    for (var i = 0; i < fog.length; i++) {
      if (fog[i].kind !== 'surface') continue;
      var fo = fog[i], sy = fo.y - off * 0.85 + Math.sin(t * 0.2 + fo.ph) * 8;
      if (sy < -90 || sy > H + 90) continue;
      var hh = 54, col = '150,178,184', a = fo.a * (flash > 0.2 ? 1.8 : 1);
  ...
  function frame(now) {
    ...
    if (!reduce && !mobile) { nextBolt -= dt; if (nextBolt <= 0) { fireBolt(); nextBolt = rand(4, 11); } }
    ...
    if (surfScreen > -60) {
      ...
      ctx.globalAlpha = 1;
      drawLandmass(off); drawClouds(off); drawSkyFog(off); drawBeam(off);
      if (!reduce) {
        var R = 54;
